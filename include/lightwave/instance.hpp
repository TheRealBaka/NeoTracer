...
/**
 * @brief An instance represents an instantiation of a @ref Shape in your scene,
 * and binds materials and a transformation to it. Wrapping a shape like this
 * might seem cumbersome (why not add materials or transforms to the Shape class
 * itself instead?), but allows for more flexibility and efficiency when
 * building scenes. Most notably, you could load a complex triangle mesh (e.g.,
 * a tree) once, and render an entire forest by instantiating it with different
 * transformations, without increasing loading time or memory consumption.
 *
 * @note Instances can be used anywhere shapes can be used, and only differ in
 * that they augment the intersect method of the wrapped shape (namely by
 * applying an optional transform before intersecting the wrapped object, and by
 * populating the instance field).
 */
class Instance : public Shape {
    /// @brief When an instance is wrapped within an area light object, this
    /// will reference it.
    Light *m_light;
    /// @brief The shape wrapped by the instance.
    ref<Shape> m_shape;
    /// @brief The material that the shape should be rendered with (can be null
    /// for non-reflecting objects).
    ref<Bsdf> m_bsdf;
    /// @brief The distribution of light the shape should emit (can be null for
...
public:
    ...
    /// @brief Returns the material that the shape should be rendered with (can
    /// be null for non-reflecting objects).
    Bsdf *bsdf() const { return m_bsdf.get(); }
    /// @brief Returns the distribution of light the shape should emit (can be
    /// null for non-emissive objects).
    ...
    void markAsVisible() override { m_visible = true; }

    /// @brief Sets the parent light object that contains this instance.
    void setLight(Light *light) {
        if (m_light) {
            lightwave_throw(
                "instances can only have one light associated with "
                "them, %s has multiple!",
                indent(this));
        }
        m_light = light;
     ...
     * @param its Contains the intersection if one occured, otherwise left
     * unchanged.
    ...
    /**
     * @brief Samples a point in world coordinates on the surface of this
     * instance.
     * @param rng A random number generator used to steer sampling decisions.
     */
